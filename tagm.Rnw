...
each with $10$ fractions along the density gradient. {\color{red}{We defined gold
...
where the pointer {\color{red}{is scaled according to the localisation
...
{\color{red}{(see Section \ref{section:methods})}}.
...
We find that in 16 out of 19 datasets {\color{red}{(all of those except HeLa Wild type,
HeLa KO1 and HeLa K02)}} the MCMC methods achieves the
...
{\color{red}{We note that our method is general enough
...
{\color{red}{Currently, our model cannot integrate localisation information 
from different data sources nor can it explicitly model proteins with multiple 
localisation. We cannot model proteins with multiple locations because our model
is mathematically constrained such that proteins can only be allocated to a single
class. However, one (of many) biological explanations for the uncertainty in 
these allocation probabilities is multiple localisation. Thus a protein which
is uncertain between two sub-cellular niches is perhaps resident of both niches.
In further work, we plan to explicitly model such cases to deconvolute different
sources of uncertainty. In addition, extensions to semi-supervised non-parametric
methods are under consideration to detect novel sub-cellular niches. 
These are the subjects of further work.}}
...
\citep{Lonnberg:2017} {\color{red}{and affinity purification MS proteomics}}
\citep{Choi:2010}.
...
as half the global variance of the data, {\color{red}{including labelled and unlabelled proteins}}.
...
proteins throughout the text. {\color{red}{The variance of the multivariate T-distribution
