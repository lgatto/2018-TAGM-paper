...
source("./src/BayesGMM/plotEllipse.R")
...
probabilitically infer the sub-cellular localisation of thousands of
...
considered perform competively. To demonstrate the additional
...
localisation probabilites, we can draw meaningful biological results
...
proteins, simultanously. This approach reveals global patterns of
...
component as a representive example. For the TAGM model, we derive
...
<<echo=FALSE>>=
par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MAP", components = c(1,2),
...
<<echo=FALSE>>=
par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MAP", components = c(1,4),
            main = "Visualisation along 1st and 4th principle components")
...
\ref{section:methods}, to predcit the localisation of proteins to
organelles and sub-cellular components.  In brief, we produce
\textit{maximum a posteriori} (MAP) estimates of the parameters by
using the expectation-maximisation algorithm, to form the basis of a
Bayesian analysis.  We run the algorithm for $200$ iterations and
inspect a plot of the log-posterior to assess convergence of the
algorithm (see appendix \ref{app:logposterior}). We confirm that the
difference of the log posterior between the final two iterations is
less than $10^{-6}$ and we conclude that our algorithm has
converged. The results can be seen in figure
\ref{fig:assignmentPCAMAP}, where the posterior localisation
probability is visualised by scaling the pointer for each protein.

Figure \ref{fig:assignmentPCAMAP} demonstrates a range of
...
localisation probabilites. The remainder of this article focus on
analysis of spatial proteomics in a fully Bayesian framework.


\begin{figure}[ht]
<<echo = FALSE, fig.align = "center", out.width = '0.5\\linewidth'>>=
...
plot2D(object = andyhlMAP$object,
       ...
       dims = c(1,2),
       cex = ptsze,
       main="Prediction of the localisation of proteins",
       mirrorY = TRUE)
...
@
  ...
  \caption{PCA plot of the protein quantitation data with colours
    representing the predicted class (5032 proteins). The pointer size
    of a protein is scaled to the probability that particular protein
    was assigned to that organelle. Markers are automatically assigned
    a probability of $1$ and the size of the pointer reflects this.}
...
Bayesian setting using standard MCMC methods.  We run $6$ chains of
our Gibbs sampler in parallel for $15,000$ iterations, throwing away
the first $4,000$ iterations for burn-in and retain every $10^{th}$
sample for thinning.  Thus 1,100 sample are retained from each
chain. We then visualise our trace plots of our chains; in particular,
we monitor the number of proteins allocated to the known components
(see appendix \ref{app:mcmc}). We throw away $1$ chain because we do
not considered it to have converged.  For the remaining $5$ chains we
further discard the first $500$ samples by visual inspection.  We then
have $600$ retained samples from $5$ seperate chains. For further
analysis, we compute the Gelman-Rubin convergence diagnostic
\citep{Gelman:1992, Brooks:1998}, which is computed as
...
are then visualised on figure \ref{fig:assignmentPCAMCMC}, where the
pointer is scaled according to the localisation
...
capturing uncertainty.  The E3 ubiquitin-protein ligase TRIP
12(G5E870) is an integral part of ubiquitin fusion degradation pathway
...
localisation probabilities inferred from the TAGM model, shown in
...
<<echo=FALSE>>=
par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MCMC", components = c(1,2),
            main = "Visualisation of probability ellipses",
            xlim = c(-0.8,0.3), ylim = c(-0.5,0.5))
...
<<echo=FALSE>>=
par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MCMC",
            components = c(1,4),
            main = "Visualisation along the 1st and 4th principle component")
...
@
  ...
  \caption{PCA plot of the protein quantitation data with colours
    representing the predicted class (5032 proteins). The pointer size
    of a protein is scaled to the probability that particular protein
    was assigned to that organelle, with proteins belonging to the
    outlier cluster shrunk. Markers are automatically assigned a
    probability of $1$ and the size of the pointer reflects this.}
  \label{fig:assignmentPCAMCMC}
\end{figure}

\begin{figure}[ht]
\centering
<<echo = FALSE, fig.align="center", warning=FALSE, message=FALSE, out.width='0.7\\textwidth'>>=
...
using an additional outlier component described mathematically as a
t-distribution.
...
biological relevance of this component \citep{Boyle:2004, Yu:2012}.
We take proteins that were allocated to known components with
probability less than $0.95$, a total of 1111 protein.  Note that,
these 1111 proteins exclude proteins that are likely to belong to a
known location, but we are uncertain about which localisation.  We
then performed enrichment analysis against the set of all proteins
...
every fraction along the density gradient, this manifests as these
proteins not forming a tight cluster. We also observe enrichment for
highly dynamic sub-cellular processes such as cell division
...
methods on several datasets, by computing the performance metrics
...
these same time points to serves as a control - this results in $10$
...
for this datset all classifiers are performing extremely well. In the
...
CRISPR-CAS9 KO1 there are no significant difference between the KNN
...
However, the Macro-F1 scores do not take into account that whilst the
TAGM model may misclassify, it may do so with low confidence. We use
the probabilistic information that the classifiers output to compute
the quadratic loss. The lower the quadratic loss the closer the
...
and KNN classifiers. In 6 out of these 16 datasets there are no
...
In the vast majority of cases, we asserts that if the TAGM model,
...
methodology provides us with not only with point estimates of
classification probabilities but uncertainty quantification in these
allocations, and thus is is the most useful classifier.
...
levelplot(df4, xlab="SVM", ylab="TAGM (MCMC)", main=" ", scales = list(x = list(draw = FALSE)),
          cuts = 50, pretty = T, col.regions = mycol, at = seq(0, 10)/10, aspect = "fill")
@
        \caption{}
\end{subfigure}
...
matlines(t(exprs(hlo[fData(hlo)$markers == "Nucleus - Non-chromatin" , ])),lty = 1, col = getStockcol()[11], lwd = 1, type="l" )
...
names(ptsze) <- rownames(fData(andyMCMChl))
ptsze[as.numeric(fData(andyMCMChl)$outlier.allocation) <0.99995] <- 0.01
...
matlines(t(exprs(hlo[fData(hlo)$markers == "Extracellular matrix" , ])),lty = 1, col = getStockcol()[7], lwd = 1, type="l" )
...
@
 ...
 uncertainty, for example proteins in the secretary pathway or
 ...
 role. Moreover, the organelles of the secretary pathway share similar
 ...
 to undescribed sub-cellular compartments. The information portrayed
 by these plots and the conclusion therein would be extremely
...
\begin{figure}[H]
  ...
  \caption{Plate diagram for TAGM model. This diagram specifies the
    conditional independecies and parameters in our
...
Processing of the unlabelled data can be done by infering {\em maximum
...
equation consitutes a backbone of the E-step of the EM algorithm, with
...
multi-class classification is calculated using quadratic optimization.
...
for(j in 1:14){
 plist[[j]] <- pairwise.t.test(additionalF1compare[(additionalF1compare$set == levels(additionalF1compare$set)[j]),"value"], additionalF1compare[(additionalF1compare$set == levels(additionalF1compare$set)[j]),"variable"], p.adjust.method = "BH",
                           pool.sd = F, paired = FALSE,
                           alternative = "two.sided")
}
...
for(j in 1:14){
 plist[[j]] <- pairwise.t.test(additionalquadlosscompare[(additionalquadlosscompare$set == levels(additionalquadlosscompare$set)[j]),"value"],
                               additionalquadlosscompare[(additionalquadlosscompare$set == levels(additionalquadlosscompare$set)[j]),"variable"], p.adjust.method = "BH",
                           pool.sd = F, paired = FALSE,
                           alternative = "two.sided")
}
...
<<enrichgo, cache=TRUE, echo = FALSE, eval=FALSE>>=
