...
\begin{figure}[ht]
  \begin{subfigure}[t]{0.5\textwidth}
        \centering
<<fig=TRUE, echo=FALSE, fig.width = 3, fig.height = 3,fig.align="center">>=

par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MAP", components = c(1,2), main = "Visualisation of probability ellipses", xlim = c(-0.8,0.3), ylim = c(-0.5,0.5))
...
par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MAP", components = c(1,4), main = "Visualisation along 1st and 4th principle components")
...
<<fig = TRUE, echo = FALSE, fig.width = 3, fig.height = 3,fig.align="center", out.width = 0.4>>=
ptsze <- as.numeric(fData(andyhlMAP$object)$predicted.probability)
par(mfrow = c(1,1))
setStockcol(paste0(getStockcol(), 90))
plot2D(object = andyhlMAP$object, fcol="predicted.allocation", dims = c(1,2), cex = ptsze, main="Prediction of the localisation of proteins", mirrorY = TRUE )
...
<<fig=TRUE, echo=FALSE, fig.width = 3, fig.height = 3,fig.align="center">>=>>=
par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MCMC", components = c(1,2), main = "Visualisation of probability ellipses", xlim = c(-0.8,0.3), ylim = c(-0.5,0.5))
...
<<fig=TRUE, echo=FALSE, fig.width = 3, fig.height = 3,fig.align="center">>=>>=
par(mar = c(4,4,2,0))
plotEllipse(hl, method = "MCMC", components = c(1,4), main = "Visualisation along the 1st and 4th principle component")
...
\begin{figure}[ht]
\setkeys{Gin}{width=1\textwidth}
<<fig = TRUE, echo = FALSE, fig.width = 3, fig.height = 2,fig.align="center">>=>>=
...
gg <- ggplot(dfG5E870, aes(Organelle, Probability, width = (Probability))) + geom_violin(aes(fill = Organelle), scale = "width")
gg <- gg + theme_bw() + scale_fill_manual(values = getStockcol()[1:14]) + theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title.x = element_blank())
gg <- gg + ylab("Membership Probability") + ggtitle("Distribution of Subcellular Membership for Protein G5E870")
...
dfmetrics2 <- rbind(tandfmetricsquad, halldfmetricsquad, andydfmetricsquad, itzhakdfmetricsquad, u2osdfmetricsquad, additionalquadlosscompare)
...
\begin{figure}[ht]
  \begin{subfigure}[t]{0.5\textwidth}
        \centering
<<fig=TRUE, echo=FALSE, fig.width = 4, fig.height = 2,fig.align="center">>=
par(mar = c(0,0,0,0), mai = c(2,0,0,0), oma = c(0,0,0,0))
lattice.options(
  layout.heights=list(bottom.padding=list(x=0), top.padding=list(x=0)),
  layout.widths=list(left.padding=list(x=0), right.padding=list(x=0))
)
levelplot(cont, xlab="SVM", ylab="TAGM (MCMC)", main=" ", scales = list(x = list(draw = FALSE)), colorkey=FALSE,
          cuts = 50, pretty = T, col.regions = mycol, at = seq(0, 10)/10, aspect = 2)
...
\begin{subfigure}[t]{0.5\textwidth}
\centering
<<fig=TRUE, echo=FALSE, fig.width = 2 , fig.height = 2, fig.align="center">>=
par(mar = c(0,0,0,0), mai = c(2,0,0,0), oma = c(0,0,0,0))
lattice.options(
  layout.heights=list(bottom.padding=list(x=0), top.padding=list(x=0)),
  layout.widths=list(left.padding=list(x=0), right.padding=list(x=0))
...
par(mfrow = c(1,1))
setStockcol(paste0(getStockcol(), 90))
.pca <- plot2D(andyMCMChl, fcol="predicted.allocation", cex = ptsze, main="Visualising whole proteome uncertainty", mirrorY = TRUE)
addLegend(andyMCMChl, cex=0.6, where="topleft")
setStockcol(getLisacol())
@
    \caption{}
...
<<fig = TRUE, echo = FALSE, fig.width = 3, fig.height = 2,fig.align="center">>=
par(mar = c(4,4,2,0))
plot(as.numeric(fData(andyMCMChl)$predicted.probability), fData(andyMCMChl)$meanshannon, col = getStockcol()[as.factor(fData(andyMCMChl)$predicted.allocation)], pch = 19, xlab = "Localisation Probability", ylab = "Shannon Entropy", main = "Protein localistaion probability and Shannon Entropy")
@
    \caption{}
  \end{subfigure}
...
\centering
\includegraphics[width=\textwidth]{ConcludePlot.pdf}
        \caption{The barplot demonstrates the effect of applying different methodologies
        on protein assignment when applied the mouse pluripotent embryonic stem cell data. Roughly
        2000 proteins are classified using either methodology; however, TAGM can draw additional conclusions
...
We describe in this section the probabilistic model that uses the labelled
data to associate un-annotated proteins to specific organelles or sub-cellular compartments.
\subsection{Mixture models for spatial proteomic data}

We observe $N$ protein profiles each of length $L$, corresponding to the number of quantified fractions along the gradient density, including combining replicates.  For $i = 1, \ldots, N$, we denote the profile of the $i$-th protein by ${\bf x}_i = [x_{1i}, \ldots, x_{Li}]$.  We suppose that there are $K$ known sub-cellular compartments to which each protein could localise (e.g. cytoplasm, endoplasmic reticulum, mitochondria, \ldots).  Henceforth, we refer to these $K$ sub-cellular compartments as {\em components}, and introduce component labels $z_i$, so that $z_i = k$ if the $i$-th protein localises to the $k$-th component. We denote by $X_L$ the set of proteins whose component labels are known, and by $X_U$ the set of unlabelled proteins.  If protein $i$ is in $X_U$, we desire the probability that $z_i = k$ for each $k = 1, \ldots, K$.  That is, for each unlabelled protein, we want the probability of belonging to each component (given a model and the observed data).
\\
...
<<fig=TRUE, echo=FALSE>>=
plot(resandyhl$loglike, cex = 1, col = "blue", type = "b", xlab = "Iteration", ylab = "log-posterior",
