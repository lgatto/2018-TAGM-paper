...
@
  ...
  \caption{PCA plot of the protein quantitation data with colours
    ...
    was assigned to that organelle. Markers, proteins whose localisations
    are already known, are automatically assigned
...
chain. We then visualise trace plots of our chains; in particular,
we monitor the number of proteins allocated to the known components
(see appendix \ref{app:mcmc}). We discard $1$ chain because we do
...
have $600$ retained samples from $5$ separate chains. For further
...
are then visualised in figure \ref{fig:assignmentPCAMCMC}, where the
...
biological relevance of the outlier component \citep{Boyle:2004, Yu:2012}.
...
every fraction along the density gradient. We also observe enrichment for
