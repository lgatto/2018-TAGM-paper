...
{\color{red}{Currently, our model does not integrate localisation information
...
Thus a protein which is uncertain between two sub-cellular niches is perhaps resident
of both niches. In further work, we plan to explicitly model such cases
to deconvolute different sources of uncertainty. In addition, extensions
to semi-supervised non-parametric methods are under consideration to detect
novel sub-cellular niches. These are the subjects of further work.
}}
