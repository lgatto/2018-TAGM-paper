...
// [[Rcpp::export]]
double likelihoodGPcpp(arma::vec Xk,
                       arma::vec tau,
                       arma::vec h,
                       int nk,
                       int D
  ...
  negloglike = -loglikeGPcpp(Xk, Z, A, logcovDet, sigmak, nk, D)(0,0);
...
// [[Rcpp::export]]
arma::vec sampleGPmeancpp(arma::vec Xk,
                       arma::vec tau,
                       arma::vec h,
                       int nk,
                       int D
  ...
  Z = trenchInvcpp(v);
  invC = (arma::eye(D * nk , D * nk) / sigmak)  - arma::kron(J, Z * A)/(sigmak * sigmak);
  priorCor = kron(arma::ones(1,nk), A);
  postM = priorCor * (invC * Xk);
  
  postV = A - (nk * (A * A)/sigmak) + nk * nk * (A * Z * A * A)/(sigmak * sigmak);
  cholV = arma::chol(postV);
  GPmean = postM + cholV * as<arma::vec>(rnorm(D));
...
// [[Rcpp::export]]
arma::vec makeComponent(arma::mat X,
                        arma::vec BX,
                        arma::mat Y,
                        arma::vec BY,
                        int j){
  ...
  component = arma::join_cols(vecX, vecY);
...
arma::mat normalisedData(arma::mat Xknown,
  ...
  Y = makeComponent(Xknown, BX, Xunknown, BXun, j);
  sampleGPMean = sampleGPmeancpp(Y, tau, hypers, nk(j-1), D);
  out = Xunknown.each_row() - sampleGPMean.t();
